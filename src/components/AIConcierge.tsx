...
import type { Appointment, Listing, ListingInput } from "@/lib/types";
...
type ListingContext = { reference: string; title: string; location: string };
...
export function AIConcierge({ listing, adminAccess = false, initiallyOpen = false, onListingsChanged }: AIConciergeProps) {
  ...
  async function ask(content: string) {
    ...
    try {
      ...
      if (payload.uiAction?.type === "open_appointment_calendar" && listing) {
  ...
  function handleAppointmentBooked(appointment: Appointment) {
    setOpen(true);
    setMessages((current) => [...current, {
      role: "assistant",
      content: `${new Intl.DateTimeFormat("tr-TR", { timeZone: "Europe/Istanbul", weekday: "long", day: "numeric", month: "long", hour: "2-digit", minute: "2-digit" }).format(new Date(appointment.startAt))} için randevu talebiniz kaydedildi. ${appointment.status === "Talep Alındı" ? "Danışman programı doğruladıktan sonra sizinle iletişime geçecek." : "Randevunuz onaylandı."}`,
    }]);
  }
